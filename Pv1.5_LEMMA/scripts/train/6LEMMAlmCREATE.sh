...
mkdir -p $LMfolder
mkdir -p $LMlemmaFolder
for method in ${methodType[*]}
do
  for order in $(seq $orderMin $orderMax)
  do
    ngramcount --order=$order --require_symbols=false $outputFolder/LEMMAtrain.far |
    ngrammake --method=$method - > $LMlemmaFolder/lmLEMMA$order$method.lm
  done
done