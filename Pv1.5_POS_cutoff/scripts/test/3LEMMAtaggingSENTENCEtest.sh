...
rm -Rf $LemmaTaggedOutpuFolde
mkdir -p $LemmaTaggedOutpuFolde
for method in ${methodType[*]}
do
  for order in $(seq $orderMin $orderMax)
  do
    for file in $FSTsentenceTESTfolder/*; do
      fstcompose $file $outputFolder/finalLEMMAtaggerC.fst |
      fstcompose - $LemmaTaggedOutpuFolde/lmLEMMA$order$method.lm|
      fstrmepsilon -|
      fstshortestpath -|
      fsttopsort - |
      fstprint  --isymbols=$outputFolder/lex.txt  --osymbols=$outputFolder/lex.txt -|
      awk '$3 {OFS="\t";print $3,$4}'>>$LemmaTaggedOutpuFolde/orderSmooth$order$method.txt
    done
    echo "Finished test with order$order and method $method, $LemmaTaggedOutpuFolde/orderSmooth$order$method.txt"
  done
done
