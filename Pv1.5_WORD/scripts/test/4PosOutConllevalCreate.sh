...
rm -Rf $PosOUTtoConllevalFolder
mkdir -p $PosOUTtoConllevalFolder
...
do
  ...
  do
    ...
    paste $outputFolder/token $outputFolder/pos $outputFolder/POStag $outputFolder/predictedPOStag > $PosOUTtoConllevalFolder/OUTorderSmooth$order$method.txt
  done
done
