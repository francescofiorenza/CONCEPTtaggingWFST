...
ngramcount --order=1 --require_symbols=false $outputFolder/$ConceptFarName|
ngrammake --method=unsmoothed - > $LMconceptFolder/lmCONCEPT1unsmoothed.lm
echo -n "--- 11 Finished to generated the language model of the concept with order 1 and no smoothing method."
echo " $((`date +%s`-start)) sec"
start=`date +%s`
...
./$testFolder/3CONCEPT_MAP_taggingSENTENCEtest.sh
...
cat $ConceptTaggedOutpuFolder/MAP.txt|awk '$1 {print $2}' > $outputFolder/predictedCONCEPTtag
paste $outputFolder/token $outputFolder/pos $outputFolder/CONCEPTtag $outputFolder/predictedCONCEPTtag > $ConceptOUTtoConllevalFolder/OUTmap.txt

mkdir -p $ConceptPlotFolder
perl $toolsEval/conlleval.pl -d '\t' < $ConceptOUTtoConllevalFolder/OUTmap.txt > $ConceptConllevalOutFolder/accuracyMAP.txt
echo 'accuracy & precision & recall & FB1 \\'  > $ConceptPlotFolder/MAP.dat
echo -en 'MAP & '"$(cat $ConceptConllevalOutFolder/accuracyMAP.txt|grep 'accuracy: '|awk '{print $2}'|tr -d ['%;'])" >> $ConceptPlotFolder/MAP.dat
echo -en ' & '"$(cat $ConceptConllevalOutFolder/accuracyMAP.txt|grep 'accuracy: '|awk '{print $4}'|tr -d ['%;'])" >> $ConceptPlotFolder/MAP.dat
echo -en ' & '"$(cat $ConceptConllevalOutFolder/accuracyMAP.txt|grep 'accuracy: '|awk '{print $6}'|tr -d ['%;'])" >> $ConceptPlotFolder/MAP.dat
echo -en ' & '"$(cat $ConceptConllevalOutFolder/accuracyMAP.txt|grep 'accuracy: '|awk '{print $8}'|tr -d ['%;'])" >> $ConceptPlotFolder/MAP.dat
echo -en ' \\\\ ' >> $ConceptPlotFolder/MAP.dat
