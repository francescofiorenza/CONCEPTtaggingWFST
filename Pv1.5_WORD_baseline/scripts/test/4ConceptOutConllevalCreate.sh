...
rm -Rf $ConceptOUTtoConllevalFolder
...
do
  ...
  do
    ...
    cat $ConceptTaggedOutpuFolder/orderSmooth$order$method.txt|awk '$1 {print $2}' > $outputFolder/predictedCONCEPTtag
    paste $outputFolder/token $outputFolder/pos $outputFolder/CONCEPTtag $outputFolder/predictedCONCEPTtag > $ConceptOUTtoConllevalFolder/OUTorderSmooth$order$method.txt
  done
done
