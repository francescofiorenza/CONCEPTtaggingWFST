...
do
    echo -en "order-$order-cutoff1"'\t' >> $ConceptPlotFolder/accuracy.dat
done
...
for order in $(seq $orderMin $orderMax)
do
    echo -en "order-$order-cutoff1"'\t' >> $ConceptPlotFolder/precision.dat
done
...
for order in $(seq $orderMin $orderMax)
do
    echo -en "order-$order-cutoff1"'\t' >> $ConceptPlotFolder/recall.dat
done
...
for order in $(seq $orderMin $orderMax)
do
    echo -en "order-$order-cutoff1"'\t' >> $ConceptPlotFolder/FB1.dat
done
